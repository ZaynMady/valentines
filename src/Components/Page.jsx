...
export default function ValentineRequest() {
    ...
    if (!hasStarted) {
        return (
            <div style={{ backgroundColor: 'black', height: '100dvh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <button
                    onClick={startExperience}
                    style={{ padding: '20px 40px', borderRadius: '50px', border: 'none', background: '#ff4d6d', color: 'white', fontWeight: 'bold', cursor: 'pointer', fontSize: '1.2rem' }}
                >
                    Enter the Forest 🌀
                </button>
            </div>
    ...
    if (isAccepted) {
        return (
            <div style={{
                backgroundColor: '#1a1a1a', height: '100dvh', width: '100vw',
                position: 'fixed', top: 0, left: 0, overflow: 'hidden'
            }}>
                {/* Audio for Success */}
                <audio ref={winAudioRef} loop src={celebrationMusic} />

                {/* The Bouncing GIF (DVD Screensaver Style) */}
                <motion.img
                    src={dancingGifUrl}
                    style={{
                        width: '150px',
                    ...
                    animate={{
                        // This forces it to use the full width/height minus the GIF size
                ...
                {/* Main Content (Centered on top of the bouncing GIF) */}
                <div style={{
